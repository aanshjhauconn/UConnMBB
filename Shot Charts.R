...
todays_data <- todays_data[,-10]
todays_data$date <- paste0(date,'_24')
todays_data$distance <- sqrt( (todays_data$x-25)^2 + (todays_data$y-6)^2 )
todays_data$court_region <- 0
#View(todays_data)
for (i in 1:nrow(todays_data)){
  #corner 3's
  if(todays_data$y[i] <= 10){
    if(todays_data$x[i] < 3.5){
      todays_data$court_region[i] <- 'Right Corner 3'
    }else if (todays_data$x[i] > 46.5){
      todays_data$court_region[i] <- 'Left Corner 3'
    }
  }else if (3.5 <= todays_data$x[i] & todays_data$x[i] <= 15.5 & todays_data$shot_distance[i]=='3'){ #wing 3's
    todays_data$court_region[i] <- 'Right Wing 3'
  }else if (todays_data$x[i] < 3.5 & todays_data$y[i] > 10 & todays_data$shot_distance[i]=='3'){
    todays_data$court_region[i] <- 'Right Wing 3'
  }else if (34.5 <= todays_data$x[i] & todays_data$x[i] <= 46.5 & todays_data$shot_distance[i]=='3'){
    todays_data$court_region[i] <- 'Left Wing 3'
  }else if (todays_data$x[i] >= 46.5 & todays_data$y[i] >= 10 & todays_data$shot_distance[i]=='3'){
    todays_data$court_region[i] <- 'Left Wing 3'
  }else if (15.5 <= todays_data$x[i] & todays_data$x[i] <= 34.5 & todays_data$shot_distance[i]=='3'){ #straight on 3
    todays_data$court_region[i] <- 'Straight on 3'
  }else if (3.5 <= todays_data$x[i] & todays_data$x[i] <= 19 & todays_data$y[i] >= 8 & todays_data$shot_distance[i] != '3'){ #wing jumpers
    todays_data$court_region[i] <- 'Right Wing 2'
  }else if (31 <= todays_data$x[i] &todays_data$x[i] <= 46.5 & todays_data$y[i] >= 8 & todays_data$shot_distance[i] != '3'){
    todays_data$court_region[i] <- 'Left Wing 2'
  }else if (19 <= todays_data$x[i] & todays_data$x[i] <= 31 & todays_data$y[i] >= 19 & todays_data$shot_distance[i] != '3'){ #straight on jumpers
    todays_data$court_region[i] <- 'Straight on 2'
  }else if (19 <= todays_data$x[i] & todays_data$x[i] <= 25 & 14.75 <= todays_data$y[i] & todays_data$y[i] <= 19){#Deep Paint
    todays_data$court_region[i] <- 'Deep Paint Right'
  }else if (25 <= todays_data$x[i] & todays_data$x[i] <= 31 & 14.75 <= todays_data$y[i] & todays_data$y[i] <= 19){
    todays_data$court_region[i] <- 'Deep Paint Left'
  }else if (19 <= todays_data$x[i] & todays_data$x[i] <= 25 & 10 <= todays_data$y[i] & todays_data$y[i] <= 14.75){ #Mid Paint
    todays_data$court_region[i] <- 'Mid Paint Right'
  }else if (25 <= todays_data$x[i] & todays_data$x[i] <= 31 & 10 <= todays_data$y[i] & todays_data$y[i] <= 14.75){
    todays_data$court_region[i] <- 'Mid Paint Left'
  }
}
for (i in 1:nrow(todays_data)){
  if (19 <= todays_data$x[i] & todays_data$x[i] <= 25 & todays_data$y[i] <= 10){ #Short paint
    todays_data$court_region[i] <- 'Short Paint Right'
  }else if (25 <= todays_data$x[i] & todays_data$x[i] <= 31 & todays_data$y[i] <= 10){
    todays_data$court_region[i] <- 'Short Paint Left'
  }
} 
for (i in 1:nrow(todays_data)){
  if (3.5 <= todays_data$x[i] & todays_data$x[i] <= 19 & todays_data$y[i] <= 8 & todays_data$shot_distance[i] != '3'){ #baseline 2's
    todays_data$court_region[i] <- 'Right Baseline 2'
  }else if (31 <= todays_data$x[i] & todays_data$x[i] <= 46.5 & todays_data$y[i] <= 8 & todays_data$shot_distance[i] != '3'){
    todays_data$court_region[i] <- 'Left Baseline 2'
  }
}  
for (i in 1:nrow(todays_data)){
  if (todays_data$x[i] <= 25 & todays_data$shot_distance[i]=='3' & todays_data$court_region[i]==0){
    todays_data$court_region[i] <- 'Right Corner 3'
  }else if (todays_data$x[i] >= 25 & todays_data$shot_distance[i]=='3' & todays_data$court_region[i]==0){
    todays_data$court_region[i] <- 'Left Corner 3'
  }
}
for (i in 1:nrow(todays_data)){
  if (todays_data$court_region[i]==0){
    if (todays_data$x[i] <= 25){
      todays_data$court_region[i] <- 'Right Wing 2'
    }else if (todays_data$x[i] > 25){
      todays_data$court_region[i] <- 'Left Wing 2'
    }
  }
}

totals <- read.csv('/Users/aansh/OneDrive/Desktop/UConn MBB/totals.csv')
totals <- totals[,-1]
...
totals_sheet <- totals_sheet[,-1]
totals_sheet <- rbind(totals_sheet,todays_data)
totals <- rbind(totals,todays_data)
write.csv(totals_sheet,'/Users/aansh/OneDrive/Desktop/UConn MBB/fall_totals.csv')
write.csv(totals,'/Users/aansh/OneDrive/Desktop/UConn MBB/totals.csv')
write.csv(totals,'/Users/aansh/OneDrive/Desktop/UConn MBB/Graph Shots/totals.csv')
#totals_sheet <- read.csv('/Users/aansh/Desktop/UConn MBB/fall_totals.csv')
#totals_sheet <- rbind(todays_data[,-13],totals_sheet[,-1])
#write.csv(totals_sheet,'/Users/aansh/Desktop/UConn MBB/fall_totals.csv')
#todays_data <- todays_data[,-13]


#ppp with a paint touch and 3pt% off paint touch
for (i in 1:nrow(todays_data)){
...
shootingstats <- function(name,dataframe){
  data <- dataframe %>% filter(player==name)
...
graph <- function(name){
  df <- todays_data %>% filter(player==name)
  ...
  text2 <- data.frame(label=c(x1,y1),graphtype=c(date2,'Total')) #format(Sys.Date(),'%m-%d')
  text <- data.frame(label=c(x,y),graphtype=c(date2,'Total')) #format(Sys.Date(),'%m-%d')
...
shottypes <- function(name){
  df <- totals_sheet %>% filter(player==name)
...
for (name in sort(unique(totals_sheet$player))){
  print(shottypes(name))
}
print(team())
print(teamshottypes())
plot.new()
print(grid.table(todays_data_frame))
plot.new()
